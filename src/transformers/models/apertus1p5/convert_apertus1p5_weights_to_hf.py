...
2. The encode-only EMU3.5 vision tokenizer in `Apertus1p5VisionTokenizerModel` format: the original BAAI
   weights with `decoder.*`/`post_quant_conv.*` dropped, saved with `save_pretrained` (strict 1:1 load).
